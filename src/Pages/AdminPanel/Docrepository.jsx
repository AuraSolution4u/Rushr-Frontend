...
//         {Strings.you_want_to_delete_this_document}
//       </p>
...
//   useEffect(() => {
//     const getChapterRepoFilesByChapterId = async () => {
//       try {
//         const response = await axios.post(
//           "/restAPI/chapter/getChapterRepoFilesByChapterId",
//           {
//             userId: 2,
//             chapterId: 1,
//           },
...
//           setRepoFiles(response.data.responseObject);
//         } else {
...
//     getChapterRepoFilesByChapterId();
//   }, []);
...
//       fetch(documentToDownload.fileUrl)
...
//         });
//     } else {
//       const dummyContent = `This is a dummy content for ${documentToDownload.fileName}`;
...
//       <div
//         className={`grid ${isListView ? "grid-cols-1" : "grid-cols-3"} gap-4`}
//       >
//         {sectionDocuments.map((doc) => (
//           <div
//             key={doc.id}
//             className={`bg-white rounded-full p-4 flex items-center gap-3 shadow-sm ${
//               isListView ? "flex-row" : ""
//             }`}
...
//   return (
//     <div className="min-h-screen w-full scrollbar-hide overflow-scroll h-[100vh]  bg-gray-50 flex flex-col  mt-[83px]">
//       <div className="p-4 border-b border-gray-200 bg-white flex flex-row gap-[22rem] justify-between">
...
//           "Uploaded by You",
//           documents.filter((doc) => doc.section === "Uploaded by You")
//         )}

//         {renderDocumentSection(
//           "Uploaded by Members",
//           documents.filter((doc) => doc.section === "Uploaded by Members")
//         )}
//       </div>
...
//               {file.url}
//             </a>
//             {file.uploadedByYou && <span> (Uploaded by you)</span>}
//           </div>
//         ))}
//       </div>
//     </div>
...
const DocumentRepository = () => {
  ...
  useEffect(() => {
    const fetchChapterRepoList = async () => {
      try {
        ...
        const response = await getChapterRepoListByChapterId(JSON.stringify(getRequestBody));
  ...
  return (
    // <div className="h-[100vh] w-full overflow-scroll scrollbar-hide flex flex-col pr-[20px] pt-[120px] ">
    ...
    //   <div className="p-6 w-full -ml-6">
    //     {documents.length === 0 ? (
    //       <p className="text-center text-gray-500">{Strings.no_documents_found}</p>
    //     ) : (
    //       <>
    //         {renderDocumentSection(
    //           "Uploaded by You",
